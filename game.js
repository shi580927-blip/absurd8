...
const helperLayoutKey='absurd8-helper-layout-v11-landscape';
const helperLayoutDefaults={
...
function readHelperLayout(){try{return {...helperLayoutDefaults,...JSON.parse(localStorage.getItem(helperLayoutKey)||'{}')}}catch(e){return {...helperLayoutDefaults}}}
...
function saveLayoutItem(item){const roomSaved=readLayout(),key=layoutItemKey(item),previous=layoutItemPosition(roomSaved,item)||{},common={z:+item.style.zIndex||previous.z||0,hidden:previous.hidden||false};let next;if(item.dataset.layoutMode==='offset'){next={...common,dx:+(parseFloat(item.style.getPropertyValue('--layout-x'))||0).toFixed(2),dy:+(parseFloat(item.style.getPropertyValue('--layout-y'))||0).toFixed(2),scale:+(parseFloat(item.style.getPropertyValue('--layout-scale'))||1).toFixed(2)}}else{const stage=itemStage(item),stageRect=stage.getBoundingClientRect(),left=item.offsetLeft,top=item.offsetTop,width=item.offsetWidth;next={...common,left:+(left/stageRect.width*100).toFixed(2),top:+(top/stageRect.height*100).toFixed(2),width:+(width/stageRect.width*100).toFixed(2)}}if(stagedLayoutItemIds.has(item.id)){const helpers=readHelperLayout();helpers[key]=next;localStorage.setItem(helperLayoutKey,JSON.stringify(helpers));return}const custom=JSON.parse(localStorage.getItem(layoutKey())||'{}');custom[key]=next;localStorage.setItem(layoutKey(),JSON.stringify(custom))}
...
$('copyLayout').addEventListener('click',async()=>{const roomPositions=readLayout();stagedLayoutItemIds.forEach(id=>delete roomPositions[id]);const value=JSON.stringify({device:layoutProfile(),room:activeRoomStage+1,helpers:readHelperLayout(),positions:roomPositions},null,2);try{await navigator.clipboard.writeText(value);$('layoutStatus').textContent='координаты и размеры скопированы'}catch(e){prompt('Скопируйте координаты:',value)}});

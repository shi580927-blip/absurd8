...
function renderCare(){updateCare();['hunger','mood','rest'].forEach(key=>{const value=Math.round(state.care[key]);$(`${key}Bar`).style.width=`${value}%`;$(`${key}Value`).textContent=`${value}%`});const request=state.care.request?careRequests[state.care.request]:null;$('openCare').classList.toggle('has-request',!!request);$('careRequestIcon').textContent=request?.icon||'🐾';$('careRequestTitle').textContent=request?.title||'Шеф обдумывает пожелания';$('careRequestText').textContent=request?.text||'Он сообщит, когда потребуется персонал.';$('careAction').textContent=request?.action||'Ожидаем распоряжений';$('careAction').disabled=!request;const remaining=Math.max(0,state.care.bonusUntil-Date.now());$('careBonus').classList.toggle('active',remaining>0);$('careBonus').textContent=remaining>0?`Забота одобрена: доход ×2 ещё ${Math.ceil(remaining/60000)} мин.`:'Бонус заботы пока не действует.';const adRemaining=Math.max(0,state.adBonusUntil-Date.now());document.querySelector('.rewarded-ad').classList.toggle('active',adRemaining>0);$('rewardedAd').disabled=adRemaining>0;$('rewardedAd').textContent=adRemaining>0?'Ускорение уже работает':'Посмотреть рекламу · доход ×3';$('adStatus').textContent=adRemaining>0?`Министерское ускорение действует ещё ${Math.ceil(adRemaining/60000)} мин.`:'Реклама откроется только после нажатия.'}
...
$('careAction').addEventListener('click',()=>{updateCare();const type=state.care.request;if(!type)return;if(type==='hunger'){const cost=Math.max(10,perClick()*8);if(state.food<cost){$('careRequestText').textContent=`Для особого перекуса не хватает ${format(cost-state.food)} рыбов.`;return}state.food-=cost}state.care[type]=Math.min(100,state.care[type]+32);state.care.request=null;state.care.bonusUntil=Date.now()+5*60*1000;state.care.nextRequest=Date.now()+(2+Math.random())*3600000;$('phrase').textContent=type==='hunger'?'Особый перекус принят. Шеф великодушно не оставил ни крошки.':type==='mood'?'Перо побеждено. Настроение руководства улучшилось.':'Тишина объявлена государственной необходимостью.';save();render(true)});
$('rewardedAd').addEventListener('click',()=>{if(state.adBonusUntil>Date.now())return;if(!ysdk?.adv){$('adStatus').textContent='Реклама будет доступна после запуска игры на Яндекс Играх.';return}$('rewardedAd').disabled=true;$('adStatus').textContent='Министерство подбирает достойный рекламный ролик…';let rewarded=false;ysdk.adv.showRewardedVideo({callbacks:{onOpen:()=>{$('adStatus').textContent='Просмотр начался. Награда будет выдана после завершения.'},onRewarded:()=>{rewarded=true;state.adBonusUntil=Date.now()+5*60*1000;save()},onClose:()=>{$('phrase').textContent=rewarded?'Реклама просмотрена. Доход Шефа увеличен в 3 раза.':'Просмотр не завершён — ускорение не начислено.';render(true)},onError:()=>{$('adStatus').textContent='Сейчас реклама недоступна. Попробуйте немного позже.';$('rewardedAd').disabled=false}}})});
...
setInterval(()=>{const gain=cps()/10;state.food+=gain;state.total+=gain;render()},100);setInterval(()=>{updateCare();save();if($('care').classList.contains('open'))renderCare();else $('openCare').classList.toggle('has-request',!!state.care.request)},60000);setInterval(save,5000);addEventListener('beforeunload',save);updateCare();render(true);applyLayout();scheduleRoomEvent(true);

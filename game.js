...
let thoughtTimer,thoughtSide=false,previousThoughtMessage='';
...
function initCatThoughts(){['assets/images/reaction-happy.webp','assets/images/reaction-fussy.webp'].forEach(src=>{const preload=new Image();preload.src=src});new MutationObserver(()=>{const message=$('phrase').textContent;if(message===previousThoughtMessage)return;previousThoughtMessage=message;if(message.startsWith('Шеф отвернулся')||message.startsWith('Шеф демонстративно')){const text=Math.random()<.22?'Ой, бабочка… Шеф уже забыл, что заказывал.':'Шеф передумал. Унесите это немедленно.';showCatThought('fussy',text)}else if(message.includes('подано. Ковёр')||message.startsWith('Шеф принял блюдо'))showCatThought('happy','Котик доволен. Можно продолжать обслуживание.')}).observe($('phrase'),{childList:true,characterData:true,subtree:true})}

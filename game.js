...
let ysdk=null,gameplayActive=false,loadingReady=false,brandIntroFinished=false,platformPaused=false;
...
function queueCatThought(refused){thoughtBatch.count++;thoughtBatch.refused||=refused;clearTimeout(thoughtBatchTimer);thoughtBatchTimer=setTimeout(()=>{const batch=thoughtBatch;thoughtBatch={count:0,refused:false};if(batch.count>1)showCatThought(batch.refused?'fussy':'happy',batch.refused?'Шеф ознакомился с меню. Некоторые позиции велел унести.':'Шеф ознакомился с меню и одобрил выбор.');else if(batch.refused)showCatThought('fussy',Math.random()<.22?'Ой, бабочка… Шеф уже забыл, что заказывал.':'Шеф передумал. Унесите это немедленно.');else showCatThought('happy','Котик доволен. Можно продолжать обслуживание.')},800)}
function initCatThoughts(){['assets/images/reaction-happy.webp','assets/images/reaction-fussy.webp'].forEach(src=>{const preload=new Image();preload.src=src});new MutationObserver(()=>{const message=$('phrase').textContent;if(message===previousThoughtMessage)return;previousThoughtMessage=message;if(message.startsWith('Шеф отвернулся')||message.startsWith('Шеф демонстративно'))queueCatThought(true);else if(message.includes('подано. Ковёр')||message.startsWith('Шеф принял блюдо'))queueCatThought(false)}).observe($('phrase'),{childList:true,characterData:true,subtree:true})}
...
async function initYandexSDK(){try{if(window.YaGames){ysdk=await YaGames.init();const sdkLanguage=ysdk.environment.i18n.lang;applyGameLanguage(sdkLanguage);ysdk.on?.('game_api_pause',handlePlatformPause);ysdk.on?.('game_api_resume',handlePlatformResume);announceGameReady()}}catch(error){ysdk=null}}
initYandexSDK();
const careRequests={
...
function feed(e){if(gameIsPaused()||document.querySelector('.game').classList.contains('layout-mode'))return;const before=currentLevel();state.food+=perClick();state.total+=perClick();const after=currentLevel();if(after<=before){playFeedSound();if(after===levels.length-1)playPurr()}const cat=$('cat'),bowl=$('bowl');cat.classList.add('bop');bowl.classList.add('served');setTimeout(()=>{cat.classList.remove('bop');bowl.classList.remove('served')},180);if(after>before){$('phrase').textContent=after===12?'Шеф официально перешёл к заслуженному отдыху. Государственные дела подождут.':after===13?'Достигнута абсолютная сытость. Шеф доволен и продолжает принимать рыбов.':`Новый статус: «${levels[after].name}». Шеф ожидал этого раньше.`;lastClickPhrase=Date.now()}else if(Date.now()-lastClickPhrase>18000&&Math.random()<.06){$('phrase').textContent=phrases[Math.floor(Math.random()*phrases.length)];lastClickPhrase=Date.now()}const f=document.createElement('span');f.className='floater';f.textContent=`+${format(perClick())} 🐟`;f.style.left=`${e?.clientX||innerWidth/2}px`;f.style.top=`${e?.clientY||innerHeight/2}px`;$('floaters').append(f);setTimeout(()=>f.remove(),850);render(after>before)}
...
$('treats').addEventListener('click',e=>{const button=e.target.closest('.treat-card');if(!button)return;const index=+button.dataset.treat;if(isTreatUnlocked(index))return;e.stopImmediatePropagation();const treat=carpetFood[index];confirmRewardedAction(`Открыть «${treat.name}» за рекламу?`,'Посмотрите ролик полностью — блюдо откроется навсегда, и его можно будет покупать за рыбов.',()=>showRewardedAction({event:'treat_unlock',onReward:()=>{state.adTreatUnlocks.push(index);if(state.treatWish?.index===index){state.treatWish=null;state.nextTreatWish=Date.now()+(6+Math.random()*4)*60000}},success:`«${treat.name}» открыто навсегда. Теперь его можно купить за рыбов.`}))},true);
$('treats').addEventListener('click',e=>{const button=e.target.closest('.treat-card');if(!button)return;updateCare();const index=+button.dataset.treat,treat=carpetFood[index],cost=treatPrice(treat);if((state.treatUntil[index]||0)>Date.now()){$('phrase').textContent=`«${treat.name}» уже подано. Сначала Шеф должен закончить трапезу.`;return}if(state.food<cost){playSound('error',.45);$('phrase').textContent=`Для «${treat.name}» не хватает ${format(cost-state.food)} рыбов.`;return}if(state.care.hunger>75&&Math.random()<.18){playSound('cat-soft',.6);$('cat').classList.add('refuses');$('bowl').classList.add('refused');$('phrase').textContent='Шеф отвернулся: сейчас он сыт даже для собственного аппетита. Рыбов не списано.';setTimeout(()=>{$('cat').classList.remove('refuses');$('bowl').classList.remove('refused')},850);trackEvent('treat_refused',{treat:treat.name});return}state.food-=cost;state.care.hunger=Math.min(100,state.care.hunger+10);state.care.mood=Math.min(100,state.care.mood+4);state.treatUntil[index]=Math.max(Date.now(),state.treatUntil[index]||0)+treat.minutes*60000;$('cat').classList.add('feasting');$('bowl').classList.add('served');setTimeout(()=>{$('cat').classList.remove('feasting');$('bowl').classList.remove('served')},650);playFeedSound();setTimeout(()=>playPurr(),450);$('phrase').textContent=`«${treat.name}» подано. Ковёр временно стал фуршетным залом.`;trackEvent('treat_served',{treat:treat.name,minutes:treat.minutes});save();render(true)});
...
function showAdDrawer(){if(gameIsPaused()||state.adBonusUntil>Date.now())return false;clearTimeout(adAutoPeekTimer);$('adDrawer').classList.remove('peek');adAutoPeekTimer=setTimeout(peekAdDrawer,10000);return true}
...
function showRewardedAction({event,onReward,success}){if(adRequestPending)return;const remaining=AD_WATCH_COOLDOWN-(Date.now()-lastAdWatch);if(remaining>0){playSound('error',.45);$('phrase').textContent=`Следующая реклама будет доступна через ${Math.ceil(remaining/1000)} сек.`;return}if(!ysdk?.adv){playSound('error');$('phrase').textContent='Реклама будет доступна после запуска игры на Яндекс Играх.';return}adRequestPending=true;trackEvent(`${event}_clicked`);renderAd();let rewarded=false;ysdk.adv.showRewardedVideo({callbacks:{onOpen:()=>{lastAdWatch=Date.now();localStorage.setItem('absurd8-last-ad-watch',String(lastAdWatch));pauseGameForAd()},onRewarded:()=>{rewarded=true;onReward();trackEvent(`${event}_rewarded`);save()},onClose:()=>{adRequestPending=false;resumeGameAfterAd();if(rewarded)playSound('reward');$('phrase').textContent=rewarded?success:'Просмотр не завершён — награда не выдана.';render(true)},onError:()=>{adRequestPending=false;resumeGameAfterAd();playSound('error');$('phrase').textContent='Сейчас реклама недоступна. Попробуйте немного позже.';render(true)}}})}
...
function save(){if(suppressSave)return;state.last=Date.now();localStorage.setItem(saveKey,JSON.stringify(state))}
...
setInterval(()=>{if(gameIsPaused())return;const gain=cps()/10;state.food+=gain;state.total+=gain;render()},100);setInterval(()=>{if(gameIsPaused())return;updateCare();save();if($('care').classList.contains('open'))renderCare();else $('openCare').classList.toggle('has-request',!!state.care.request)},60000);setInterval(save,5000);addEventListener('beforeunload',save);updateCare();render(true);applyLayout();initTestMode();initCatThoughts();scheduleRoomEvent(true);scheduleAdDrawer();setTimeout(()=>trackEvent('session_30_sec'),30000);setTimeout(()=>trackEvent('session_1_min'),60000);setTimeout(()=>trackEvent('session_3_min'),180000);setTimeout(()=>trackEvent('session_5_min'),300000);setTimeout(()=>$('introSplash').classList.add('hide'),3000);setTimeout(()=>{const splash=$('introSplash');splash?.remove();brandIntroFinished=true;announceGameReady();trackEvent('game_start',{level:currentLevel()+1,test_mode:testMode})},3900);

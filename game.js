...
function activeServedTreat(){const now=Date.now();let active=null;carpetFood.forEach((treat,index)=>{const until=state.treatUntil[index]||0;if(until>now&&(!active||until>active.until))active={index,treat,until}});return active}
...
function applyLayout(){const saved=readLayout(),game=document.querySelector('.game'),gameRatio=(game.clientWidth||1600)/(game.clientHeight||900);layoutItems.forEach(item=>{const pos=saved[item.id];if(!pos)return;item.style.zIndex=pos.z??'';item.style.visibility=pos.hidden?'hidden':'';if(item.dataset.layoutMode==='offset'){item.style.setProperty('--layout-x',`${pos.dx||0}vw`);item.style.setProperty('--layout-y',`${pos.dy||0}dvh`);item.style.setProperty('--layout-scale',pos.scale||1);return}const isFood=item.classList.contains('food-prop'),safeLeft=isFood?Math.max(0,Math.min(100-(pos.width||10),pos.left)):pos.left,safeTop=isFood?Math.max(5,Math.min(90,pos.top)):pos.top;item.style.left=`${safeLeft}%`;item.style.top=`${safeTop}%`;item.style.right='auto';item.style.bottom='auto';item.style.width=`${pos.width}%`;item.style.height=item.id==='laserDecor'?`${pos.width}%`:'auto';if(item.id==='boxDecor'){const finalAspect=545/475,finalHeight=(pos.width||13.65)*gameRatio*finalAspect;item.style.top='auto';item.style.bottom=`${100-pos.top-finalHeight}%`}})}

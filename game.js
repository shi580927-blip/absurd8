...
function renderCare(updateTreats=false){updateCare();['hunger','mood','rest'].forEach(key=>{const value=Math.round(state.care[key]);$(`${key}Bar`).style.width=`${value}%`;$(`${key}Value`).textContent=`${value}%`});const request=state.care.request?careRequests[state.care.request]:null;$('openCare').classList.toggle('has-request',!!request);$('careRequestIcon').textContent=request?.icon||'🐾';$('careRequestTitle').textContent=request?(gameLanguage==='en'?request.enTitle:request.title):L('Шеф обдумывает пожелания','Chef Is Considering His Demands');$('careRequestText').textContent=request?(gameLanguage==='en'?request.enText:request.text):L('Он сообщит, когда потребуется персонал.','He will notify the staff when needed.');$('careAction').textContent=request?(gameLanguage==='en'?request.enAction:request.action):L('Ожидаем распоряжений','Awaiting orders');$('careAction').disabled=!request;const remaining=Math.max(0,state.care.bonusUntil-Date.now());$('careBonus').classList.toggle('active',remaining>0);$('careBonus').textContent=remaining>0?L(`Забота одобрена: доход ×2 ещё ${Math.ceil(remaining/60000)} мин.`,`Care approved: income ×2 for ${Math.ceil(remaining/60000)} more min.`):L('Бонус заботы пока не действует.','Care bonus is not active.');if(updateTreats||!$('treats').children.length)$('treats').innerHTML=carpetFood.map((treat,index)=>{const left=Math.max(0,(state.treatUntil[index]||0)-Date.now()),active=left>0,unlocked=isTreatUnlocked(index),premium=isPremiumTreat(index),cost=treatPrice(treat);return `<button class="treat-card ${active?'active':!unlocked?`ad-locked ${premium?'secret':''}`:state.food<cost?'locked':''}" data-treat="${index}"><img src="${treatCardImage(treat)}" alt=""><span><b>${unlocked||!premium?itemName(treat):L('Секретное блюдо','Secret dish')}</b><small>${left?L(`На ковре ещё ${Math.ceil(left/60000)} мин.`,`${Math.ceil(left/60000)} min. left on the carpet`):unlocked?L(`Эффект на ${treat.minutes} мин.`,`Effect for ${treat.minutes} min.`):L(`Откроется на уровне ${treatUnlockLevels[index]}`,`Unlocks at level ${treatUnlockLevels[index]}`)}</small></span><strong>${active?L('УЖЕ ПОДАНО','ALREADY SERVED'):unlocked?`🐟 ${format(cost)}`:L('🎬 ОТКРЫТЬ ЗА РЕКЛАМУ','🎬 UNLOCK WITH AD')}</strong></button>`}).join('')}

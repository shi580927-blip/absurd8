...
function renderAd(){const remaining=Math.max(0,state.adBonusUntil-Date.now()),button=$('rewardedAd');button.classList.toggle('active',remaining>0);button.disabled=remaining>0||adRequestPending;if(remaining>0)button.innerHTML=`<span>🐟 Наедание ×3</span><small>Осталось ${Math.ceil(remaining/60000)} мин.</small>`;else if(adRequestPending)button.innerHTML='<span>🎬 Загружаем рекламу…</span><small>Награда после просмотра</small>';else button.innerHTML='<span>🎬 Посмотреть рекламу</span><small>Наедание ×3 на 5 минут</small>'}
...
let roomEventTimer;
function pauseGameForAd(){adPlaying=true;clearTimeout(roomEventTimer);$('roomEvent').classList.remove('show')}
function resumeGameAfterAd(){if(!adPlaying)return;adPlaying=false;scheduleRoomEvent()}
...
setInterval(()=>{if(adPlaying)return;const gain=cps()/10;state.food+=gain;state.total+=gain;render()},100);setInterval(()=>{if(adPlaying)return;updateCare();save();if($('care').classList.contains('open'))renderCare();else $('openCare').classList.toggle('has-request',!!state.care.request)},60000);setInterval(save,5000);addEventListener('beforeunload',save);updateCare();render(true);applyLayout();scheduleRoomEvent(true);

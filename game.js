...
const carpetFood=[
  ...
  {img:'buffets/sushi-ship-full',asset:'assets/images/buffets/sushi-ship-full.webp',buffetKey:'sushi-ship',name:'Суши-корабль Его Наглейшества',minimumPrice:3000,minutes:3,cost:140,buffet:true,premium:true,clickMultiplier:2,stageClicks:[14,34],revealLevel:8,adLevel:8,reaction:'shock',serve:['Это всё мне? Можете не отвечать.','Is all this for me? No need to answer.']},
  {asset:'assets/images/premium-dishes/royal-lobster.webp',name:'Королевский омар',minimumPrice:5000,minutes:6,cost:190,premium:true,revealLevel:9,adLevel:9,reaction:'hunting',serve:['Омар прибыл. Корона остаётся у меня.','The lobster has arrived. The crown stays with me.'],refuseReaction:'fussy',refuse:['Королевский — это про меня. Омар может подождать.','Royal refers to me. The lobster can wait.']},
  {asset:'assets/images/premium-dishes/imperial-sturgeon.webp',name:'Императорский осётр',minimumPrice:10000,minutes:7,cost:250,premium:true,revealLevel:10,adLevel:10,reaction:'judging',serve:['Осётр принят в высшее общество.','The sturgeon has been admitted to high society.'],refuseReaction:'fussy',refuse:['Императорский? Предъявите родословную.','Imperial? Show me its pedigree.']},
  {asset:'assets/images/premium-dishes/shrimp-caviar-tower.webp',name:'Башня из креветок с икрой',minimumPrice:20000,minutes:7,cost:330,premium:true,revealLevel:11,adLevel:11,reaction:'shock',serve:['Башня устойчива. Пока я не начал.','The tower is stable. Until I begin.'],refuseReaction:'fussy',refuse:['Слишком высокая. Подвиньте поближе.','Too tall. Move it closer.']},
  {asset:'assets/images/buffets/pearl-shell-full.webp',buffetKey:'pearl-shell',name:'Перламутровая раковина',minimumPrice:30000,minutes:8,cost:430,buffet:true,premium:true,clickMultiplier:2.5,stageClicks:[18,42],revealLevel:11,adLevel:11,reaction:'stunned',serve:['Жемчуг оставьте. Я веду учёт.','Leave the pearls. I keep records.']},
  {asset:'assets/images/premium-dishes/crystal-sashimi-fish.webp',name:'Хрустальная рыба с сашими',minimumPrice:60000,minutes:8,cost:560,premium:true,revealLevel:13,adLevel:13,reaction:'hunting',serve:['Хрусталь не есть. Сашими — дважды.','Do not eat the crystal. Double the sashimi.'],refuseReaction:'fussy',refuse:['Она прозрачная. Это подозрительно.','It is transparent. Suspicious.']},
  {asset:'assets/images/premium-dishes/golden-tier-stand.webp',name:'Золотая этажерка',minimumPrice:45000,minutes:9,cost:720,premium:true,revealLevel:12,adLevel:12,reaction:'stunned',serve:['Три этажа. Наконец разумная порция.','Three tiers. Finally, a sensible portion.'],refuseReaction:'fussy',refuse:['Золото одобряю. Комплектацию — нет.','Gold approved. Assortment rejected.']},
  {asset:'assets/images/premium-dishes/caviar-palace.webp',name:'Икорный дворец',minimumPrice:250000,minutes:10,cost:920,premium:true,revealLevel:13,adLevel:13,reaction:'shock',serve:['Дворец принят. Заселяюсь немедленно.','Palace accepted. Moving in immediately.'],refuseReaction:'fussy',refuse:['Дворец маловат. Я похож на арендатора?','The palace is too small. Do I look like a tenant?']},
  {asset:'assets/images/premium-dishes/truffle-tuna.webp',name:'Тунец с трюфелями',minutes:10,cost:1180,premium:true,revealLevel:7,adLevel:7,reaction:'hunting',serve:['Тунец найден. Трюфели тоже не пропадут.','Tuna found. The truffles will not go to waste either.'],refuseReaction:'fussy',refuse:['Трюфели отвлекают от главного.','The truffles distract from what matters.']},
  {asset:'assets/images/premium-dishes/royal-seafood-feast.webp',name:'Королевский морской фуршет',minimumPrice:90000,minutes:12,cost:1500,premium:true,revealLevel:14,adLevel:14,reaction:'shock',serve:['Закройте двери. Начинается государственный приём.','Close the doors. The state reception is beginning.'],refuseReaction:'fussy',refuse:['Гостей не будет? Тогда придётся справляться самому.','No guests? Then I shall have to manage alone.']}
];
const treatUnlockLevels=[1,1,2,2,3,4,6,8,10,9,10,11,12,12,13,13,14,7,14];
...
function applyStaticTranslations(){
  ...
  $('openCare').ariaLabel=L('Как поживает Шеф','How Is Chef Doing?');$('openSettings').ariaLabel=L('Настройки','Settings');$('cat').ariaLabel=L('Покормить кота','Feed the cat');$('roomEvent').ariaLabel=L('Собрать случайный бонус','Collect a random bonus');$('rewardedAd').ariaLabel=L('Показать предложение или посмотреть рекламу','Show offer or watch ad');
...
function applyGameLanguage(language){
  ...
  if(typeof render==='function'){render(!!document.querySelector('.shop.open'));renderSettings()}
}
const format=n=>Math.floor(n).toLocaleString(NUMBER_LOCALES[gameLanguage]||NUMBER_LOCALES.ru);
const WISH_REWARD=5000;
function rememberCompletedWish(){if(!state.treatWish)return;if(!state.treatWish.rewarded){state.food+=WISH_REWARD;state.total+=WISH_REWARD;state.treatWish.rewarded=true}state.lastCompletedTreatWish={...state.treatWish,completed:true}}
function renderChefWish(markSeen=false){
  ...
  const opened=$('care').classList.contains('open');
  ...
  $('wishFood').textContent=L('Показать в разделе «Еда»','Show in Food');
  ...
  $('chefWishTask').textContent=valid?L(`Задача: купить блюдо «${itemName(carpetFood[wish.index])}».`,`Task: buy “${itemName(carpetFood[wish.index])}”.`):L('Новое желание появится здесь, когда Шеф его загадает.','Chef’s next wish will appear here.');
  ...
  $('chefWishQuote').textContent=valid?L(`«Мурр… Хочу “${itemName(carpetFood[wish.index])}”.»`,`“Purr… I want ${itemName(carpetFood[wish.index])}.”`):L('«Пока распоряжений по меню нет.»','“No menu orders for now.”');
  $('chefWishImage').src=valid?treatCardImage(carpetFood[wish.index]):'assets/images/ui/care/wish-purple-can.png';
  $('chefWishImage').alt=valid?itemName(carpetFood[wish.index]):'';
  $('chefWishReward').hidden=!valid;
  $('chefWishReward').querySelector('b').textContent=completed?L(`Награда: +${format(WISH_REWARD)} рыбов получена`,`Reward: +${format(WISH_REWARD)} fish received`):L(`Награда: +${format(WISH_REWARD)} рыбов`,`Reward: +${format(WISH_REWARD)} fish`);
  $('chefWishHint').textContent=valid&&!completed?L('Нажмите кнопку — нужное блюдо будет подсвечено в каталоге.','Tap the button to highlight the dish in the catalog.'):L('Новое желание появится, когда Шеф его загадает.','A new wish will appear when Chef makes one.');
}
...
const treatPrice=treat=>Math.max(12,treat.minimumPrice||0,Math.floor(rawPerClick()*treat.cost));
...
const soundExt='mp3';
const zoomiesAudioTracks=[1,2].map(()=>{const audio=new WebAudioTrack(`assets/audio/tygydyk-owner.${soundExt}?v=20260923-3`);audio.preload='auto';audio.volume=.96;audio.loop=false;return audio});
...
const helperStageAtlasDefs={
  ...
  mouse:{
    src:'assets/images/helpers/mice-stages.webp?v=20260923-23',width:882,height:588,
    thresholds:[1,2,4,7,10],scales:[1.3,1.55,1.95,2.35,2.75],xScales:[1,1,1,1,1],
    frames:[
      {key:'mice_01_hammock_2',x:12,y:23,w:270,h:266},
      {key:'mice_02_hammock_3',x:306,y:30,w:270,h:259},
      {key:'mice_03_gazebo',x:604,y:19,w:261,h:270},
      {key:'mice_04_house',x:13,y:313,w:267,h:270},
      {key:'mice_05_castle',x:307,y:313,w:267,h:270}
    ]
  },
  delivery:{
    src:'assets/images/helpers/delivery-stages.webp?v=20260923-23',width:852,height:852,
    thresholds:[1,2,3,4,6,8,11,15],scales:[.82,.9,.98,1.04,1.1,1.18,1.28,1.4],xScales:[1,1,1,1,1,1,1,1],
    frames:[
      {key:'delivery_01_on_foot',x:16,y:19,w:251,h:260},
      {key:'delivery_02_many_boxes',x:296,y:19,w:260,h:260},
      {key:'delivery_03_cart',x:581,y:19,w:257,h:260},
      {key:'delivery_04_scooter',x:12,y:305,w:260,h:258},
...
const helperStageAtlases={},helperStageFrameCache=new Map();
Object.entries(helperStageAtlasDefs).forEach(([kind,def])=>{
  const image=new Image();
  image.onload=()=>{def.ready=true;render(true)};
  image.onerror=()=>{def.ready=false};
  image.src=def.src;
  helperStageAtlases[kind]=image
});
const helperStageOverrides={grandma:null,mouse:null,delivery:null};
function helperStageIndexFromCount(kind,count){
  const def=helperStageAtlasDefs[kind];
  if(!def||count<=0)return 0;
  let index=0;
  def.thresholds.forEach((threshold,i)=>{if(count>=threshold)index=i});
  return Math.min(index,def.frames.length-1)
}
function helperStageIndex(kind,count){
  const override=helperStageOverrides[kind];
  if(testMode&&Number.isInteger(override))return Math.max(0,Math.min(helperStageAtlasDefs[kind].frames.length-1,override));
  return helperStageIndexFromCount(kind,count)
}
function helperStageFrameUrl(kind,index){
  const def=helperStageAtlasDefs[kind],atlas=helperStageAtlases[kind],frame=def?.frames[index];
  if(!def||!frame)return null;
  if(frame.asset)return frame.asset;
  if(!atlas||!def.ready)return null;
  const cacheKey=`${kind}:${index}`;
  if(helperStageFrameCache.has(cacheKey))return helperStageFrameCache.get(cacheKey);
  const canvas=document.createElement('canvas');
  canvas.width=frame.w;canvas.height=frame.h;
  const ctx=canvas.getContext('2d');
  ctx.clearRect(0,0,frame.w,frame.h);
  ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';
  ctx.drawImage(atlas,frame.x,frame.y,frame.w,frame.h,0,0,frame.w,frame.h);
  const url=canvas.toDataURL('image/png');
  helperStageFrameCache.set(cacheKey,url);
  return url
}
function renderStagedHelper(kind,elementId){
  const def=helperStageAtlasDefs[kind],element=$(elementId);
...
function stagedHelperVisible(kind){
  return (testMode&&Number.isInteger(helperStageOverrides[kind]))||(state.helperUntil[kind]||0)>Date.now()
}
function stepHelperStageForTest(kind,delta){
  const def=helperStageAtlasDefs[kind];
  if(!def)return;
  const current=Number.isInteger(helperStageOverrides[kind])?helperStageOverrides[kind]:helperStageIndexFromCount(kind,state.counts[kind]||0);
  const next=(current+delta+def.frames.length)%def.frames.length;
  helperStageOverrides[kind]=next;
  const names={
    grandma:['блюдо','праздничная тарелка','жаркое без стула','пир','совет бабушек'],
    mouse:['две мыши','три мыши','беседка','мышиный дом','мышиный замок'],
    delivery:['пешком','гора коробок','тележка','скутер','трицикл','фургон','грузовик','дирижабль']
  };
  $('phrase').textContent=`Тест: ${names[kind]?.[next]||'стадия'} · ${next+1}/${def.frames.length}.`;
  render(true)
}
...
function initCatThoughts(){const preload=()=>Object.values(catReactionImages).forEach(src=>{const image=new Image();image.src=src});if('requestIdleCallback'in window)requestIdleCallback(preload,{timeout:6000});else setTimeout(preload,4500);new MutationObserver(()=>{const message=$('phrase').textContent;if(message===previousThoughtMessage)return;previousThoughtMessage=message;if(message.startsWith('Шеф демонстративно')||message.startsWith('Chef made a show'))queueCatThought(true);else if(message.startsWith('Шеф принял блюдо')||message.startsWith('Chef accepted'))queueCatThought(false)}).observe($('phrase'),{childList:true,characterData:true,subtree:true})}
...
function careCopy(key,value){const high=value>=75,medium=value>=45;if(key==='hunger')return high?[L('Отлично!','Excellent!'),L('Шеф сыт и доволен','Chef is full and content')]:medium?[L('Неплохо','Not bad'),L('Шеф не откажется от блюда','Chef would welcome a dish')]:[L('Пора кормить','Time to feed'),L('Шеф требует внимания к миске','Chef demands bowl attention')];if(key==='mood')return high?[L('Прекрасное!','Wonderful!'),L('Шеф в отличном настроении','Chef is in a wonderful mood')]:medium?[L('Спокойное','Calm'),L('Шеф сохраняет достоинство','Chef maintains his dignity')]:[L('Нужно порадовать','Needs cheering up'),L('Шефу не хватает развлечений','Chef needs entertainment')];return high?[L('Отлично!','Excellent!'),L('Шеф полон сил','Chef is full of energy')]:medium?[L('Нормально','Fine'),L('Шеф немного устал','Chef is a little tired')]:[L('Нужно отдохнуть','Needs rest'),L('Шефу пора подремать','Chef needs a nap')];}
function renderCare(updateTreats=false){updateCare();['hunger','mood','rest'].forEach(key=>{const value=Math.round(state.care[key]),[status,note]=careCopy(key,value),card=document.querySelector(`.care-card[data-care="${key}"]`);$(`${key}Value`).textContent=`${value}%`;$(`${key}Status`).textContent=status;$(`${key}Note`).textContent=note;card.dataset.level=value>=75?'high':value>=45?'medium':'low'});const request=state.care.request?careRequests[state.care.request]:null;renderChefWish();$('careRequestIcon').textContent=request?.icon||'🐾';$('careRequestTitle').textContent=request?(gameLanguage==='en'?request.enTitle:request.title):L('Шеф обдумывает пожелания','Chef Is Considering His Demands');$('careRequestText').textContent=request?(gameLanguage==='en'?request.enText:request.text):L('Он сообщит, когда потребуется персонал.','He will notify the staff when needed.');$('careAction').textContent=request?(gameLanguage==='en'?request.enAction:request.action):L('Ожидаем распоряжений','Awaiting orders');$('careAction').disabled=!request;const remaining=Math.max(0,state.care.bonusUntil-Date.now());$('careBonus').classList.toggle('active',remaining>0);$('careBonus').textContent=remaining>0?L(`Забота одобрена: доход ×2 ещё ${Math.ceil(remaining/60000)} мин.`,`Care approved: income ×2 for ${Math.ceil(remaining/60000)} more min.`):L('Бонус заботы пока не действует.','Care bonus is not active.');$('careSubtitle').textContent=L('Забота делает Шефа счастливее!','Care makes Chef happier!')}
...
function showLevelCelebration(levelIndex){clearTimeout(levelCelebrationTimer);stopGameplay();EventDirector.begin('level',3000);$('levelCelebrationTitle').textContent=`${levelIndex+1} · ${itemName(levels[levelIndex])}`;$('levelCelebration').classList.remove('show');void $('levelCelebration').offsetWidth;$('levelCelebration').classList.add('show');levelCelebrationTimer=setTimeout(()=>{$('levelCelebration').classList.remove('show');EventDirector.end('level');if(!document.querySelector('.shop.open'))startGameplay()},2700)}
function returnToSceneForLevel(levelIndex){document.querySelectorAll('.shop.open').forEach(panel=>{panel.classList.remove('open');panel.setAttribute('aria-hidden','true')});document.querySelector('.game').classList.remove('menu-open');document.querySelectorAll('.nav-button').forEach(button=>button.classList.toggle('active',button.id==='openFeed'));playSound('level',.9);showLevelCelebration(levelIndex);setTimeout(()=>playPurr(true),2100)}
let renderedLevel=currentLevel();
let renderedBoxStage=-1;
function render(updatePanels=false){
  const gameRoot=document.querySelector('.game');if(gameRoot.classList.contains('menu-open')&&!document.querySelector('.shop.open'))gameRoot.classList.remove('menu-open');
  ...
  if(updatePanels&&$('shop').classList.contains('open'))$('upgrades').innerHTML=upgrades.map(u=>`<button class="upgrade ${state.food<price(u)?'locked':''}" data-id="${u.id}"><span class="icon"><img src="${u.iconAsset}" alt="" draggable="false"><span class="upgrade-level">${L('Ур.','Lvl')} ${state.counts[u.id]}</span></span><span class="upgrade-copy"><b>${itemName(u)}</b><small>${itemDesc(u)}</small></span><span class="price">${L('Купить','Buy')}: ${format(price(u))} 🐟</span></button>`).join('');
  ...
  renderStagedHelper('grandma','grandmaHelper');
  $('grandmaHelper').classList.toggle('visible',stagedHelperVisible('grandma'));
  ...
  renderStagedHelper('delivery','deliveryHelper');
  $('deliveryHelper').classList.toggle('visible',stagedHelperVisible('delivery'));
  renderStagedHelper('mouse','mouseDecor');
  $('mouseDecor').classList.toggle('visible',stagedHelperVisible('mouse'));
...
$('openCare').addEventListener('click',()=>{showPanel('care');renderChefWish(true);save()});
...
const helperLayoutRoomDefaults={
  ...
  1:{
    grandmaHelper:{z:1,hidden:false,left:23.32,top:31.23,width:12.11},
...
function readHelperStageLayouts(){
  ...
  try{
    const saved=JSON.parse(localStorage.getItem(helperStageLayoutKey())||'{}');
    const merged={};
    for(const [itemId,stages] of Object.entries(defaults))merged[itemId]={...stages,...(saved[itemId]||{})};
    for(const [itemId,stages] of Object.entries(saved))if(!merged[itemId])merged[itemId]={...stages};
    return merged
  }catch(e){return structuredClone(defaults)}
}
function helperStageLayoutPosition(item){
  const stage=Number(item?.dataset?.helperStage||0);
  return readHelperStageLayouts()?.[item?.id]?.[stage]||null
}
...
function saveLayoutItem(item){const roomSaved=readLayout(),key=layoutItemKey(item),previous=layoutItemPosition(roomSaved,item)||{},common={z:+item.style.zIndex||previous.z||0,hidden:previous.hidden||false};let next;if(item.dataset.layoutMode==='offset'){next={...common,dx:+(parseFloat(item.style.getPropertyValue('--layout-x'))||0).toFixed(2),dy:+(parseFloat(item.style.getPropertyValue('--layout-y'))||0).toFixed(2),scale:+(parseFloat(item.style.getPropertyValue('--layout-scale'))||1).toFixed(2)}}else{const stage=itemStage(item),stageRect=stage.getBoundingClientRect(),left=item.offsetLeft,top=item.offsetTop,width=item.offsetWidth;next={...common,left:+(left/stageRect.width*100).toFixed(2),top:+(top/stageRect.height*100).toFixed(2),width:+(width/stageRect.width*100).toFixed(2)}}if(stagedLayoutItemIds.has(item.id)){if(hasSeparateHelperStageLayout(item)){const all=readHelperStageLayouts(),stageNumber=Number(item.dataset.helperStage||0);all[item.id]={...(all[item.id]||{}),[stageNumber]:next};localStorage.setItem(helperStageLayoutKey(),JSON.stringify(all));return}const helpers=readHelperLayout();helpers[key]=next;localStorage.setItem(helperLayoutKey(),JSON.stringify(helpers));return}const custom=JSON.parse(localStorage.getItem(layoutKey())||'{}');custom[key]=next;localStorage.setItem(layoutKey(),JSON.stringify(custom))}
function layoutPositionText(item){if(!item)return'Выберите предмет';const pos=layoutItemPosition(readLayout(),item)||{};return item.dataset.layoutMode==='offset'?`${item.dataset.layoutName}: X ${(+pos.dx||0).toFixed(2)}% · Y ${(+pos.dy||0).toFixed(2)}% · размер ${(+pos.scale||1).toFixed(2)}`:`${item.dataset.layoutName}: X ${(+pos.left||0).toFixed(2)}% · Y ${(+pos.top||0).toFixed(2)}% · ширина ${(+pos.width||0).toFixed(2)}%`}
function updateLayoutControls(){if(!selectedLayoutItem){$('layoutStatus').textContent='Выберите предмет';$('layoutLayer').textContent='Слой: —';return}const pos=layoutItemPosition(readLayout(),selectedLayoutItem)||{},z=+selectedLayoutItem.style.zIndex||pos.z||0;$('layoutStatus').textContent=layoutPositionText(selectedLayoutItem);$('layoutLayer').textContent=`Слой: ${z}`;$('toggleLayoutVisibility').textContent=pos.hidden?'Показать':'Скрыть';$('layoutSelect').value=selectedLayoutItem.id}
...
layoutItems.forEach(item=>item.addEventListener('pointerdown',e=>{if(!document.querySelector('.game').classList.contains('layout-mode'))return;e.preventDefault();e.stopPropagation();selectLayoutItem(item);item.setPointerCapture(e.pointerId);const stageRect=itemStage(item).getBoundingClientRect(),startX=e.clientX,startY=e.clientY,startLeft=item.offsetLeft,startTop=item.offsetTop,startDx=parseFloat(item.style.getPropertyValue('--layout-x'))||0,startDy=parseFloat(item.style.getPropertyValue('--layout-y'))||0;const move=ev=>{if(item.dataset.layoutMode==='offset'){const dx=startDx+(ev.clientX-startX)/innerWidth*100,dy=startDy+(ev.clientY-startY)/innerHeight*100;item.style.setProperty('--layout-x',`${dx}vw`);item.style.setProperty('--layout-y',`${dy}dvh`);$('layoutStatus').textContent=`${item.dataset.layoutName}: X ${dx.toFixed(2)}% · Y ${dy.toFixed(2)}%`;return}const left=Math.max(0,Math.min(stageRect.width-item.offsetWidth,startLeft+(ev.clientX-startX))),top=Math.max(0,Math.min(stageRect.height-item.offsetHeight,startTop+(ev.clientY-startY)));item.style.left=`${left/stageRect.width*100}%`;item.style.top=`${top/stageRect.height*100}%`;item.style.right='auto';item.style.bottom='auto';$('layoutStatus').textContent=`${item.dataset.layoutName}: X ${(left/stageRect.width*100).toFixed(2)}% · Y ${(top/stageRect.height*100).toFixed(2)}%`};const done=()=>{item.removeEventListener('pointermove',move);saveLayoutItem(item);updateLayoutControls()};item.addEventListener('pointermove',move);item.addEventListener('pointerup',done,{once:true});item.addEventListener('pointercancel',done,{once:true})}));
function resizeSelected(delta){if(!selectedLayoutItem)return;if(selectedLayoutItem.dataset.layoutMode==='offset'){const current=parseFloat(selectedLayoutItem.style.getPropertyValue('--layout-scale'))||1,next=Math.max(.5,Math.min(1.4,current+delta/20));selectedLayoutItem.style.setProperty('--layout-scale',next);saveLayoutItem(selectedLayoutItem);$('layoutStatus').textContent=`${selectedLayoutItem.dataset.layoutName}: размер ${Math.round(next*100)}%`;return}const stageWidth=layoutStage.getBoundingClientRect().width,current=selectedLayoutItem.offsetWidth/stageWidth*100,next=Math.max(3,Math.min(55,current+delta));selectedLayoutItem.style.width=`${next}%`;selectedLayoutItem.style.height=selectedLayoutItem.id==='laserDecor'?`${next}%`:'auto';saveLayoutItem(selectedLayoutItem);$('layoutStatus').textContent=`${selectedLayoutItem.dataset.layoutName}: размер ${Math.round(next)}%`}
...
function changeSelectedLayer(delta){if(!selectedLayoutItem)return;const current=layoutItemPosition(readLayout(),selectedLayoutItem)||{};selectedLayoutItem.style.zIndex=String(Math.max(-5,Math.min(100,(+selectedLayoutItem.style.zIndex||current.z||0)+delta)));saveLayoutItem(selectedLayoutItem);updateLayoutControls()}
$('toggleLayoutVisibility').addEventListener('click',()=>{if(!selectedLayoutItem)return;saveLayoutItem(selectedLayoutItem);const key=layoutItemKey(selectedLayoutItem),current=layoutItemPosition(readLayout(),selectedLayoutItem)||{},next={...current,hidden:!current.hidden};if(stagedLayoutItemIds.has(selectedLayoutItem.id)){if(hasSeparateHelperStageLayout(selectedLayoutItem)){const all=readHelperStageLayouts(),stageNumber=Number(selectedLayoutItem.dataset.helperStage||0);all[selectedLayoutItem.id]={...(all[selectedLayoutItem.id]||{}),[stageNumber]:next};localStorage.setItem(helperStageLayoutKey(),JSON.stringify(all))}else{const helpers=readHelperLayout();helpers[key]=next;localStorage.setItem(helperLayoutKey(),JSON.stringify(helpers))}}else{const custom=JSON.parse(localStorage.getItem(layoutKey())||'{}');custom[key]=next;localStorage.setItem(layoutKey(),JSON.stringify(custom))}selectedLayoutItem.style.visibility=next.hidden?'hidden':'';updateLayoutControls()});
$('centerLayout').addEventListener('click',()=>{if(!selectedLayoutItem)return;if(selectedLayoutItem.dataset.layoutMode==='offset'){selectedLayoutItem.style.setProperty('--layout-x','0vw');selectedLayoutItem.style.setProperty('--layout-y','0dvh')}else{const stage=layoutStage.getBoundingClientRect(),width=selectedLayoutItem.offsetWidth,height=selectedLayoutItem.offsetHeight;selectedLayoutItem.style.left=`${Math.max(0,(stage.width-width)/2)/stage.width*100}%`;selectedLayoutItem.style.top=`${Math.max(0,(stage.height-height)/2)/stage.height*100}%`;selectedLayoutItem.style.right='auto';selectedLayoutItem.style.bottom='auto'}saveLayoutItem(selectedLayoutItem);$('layoutStatus').textContent=`${selectedLayoutItem.dataset.layoutName}: перемещено в центр`});
$('copyLayout').addEventListener('click',async()=>{const roomPositions=readLayout();stagedLayoutItemIds.forEach(id=>delete roomPositions[id]);const value=JSON.stringify({device:layoutProfile(),room:activeRoomStage+1,helpers:readHelperLayout(),helperStages:readHelperStageLayouts(),positions:roomPositions},null,2);try{await navigator.clipboard.writeText(value);$('layoutStatus').textContent='координаты и размеры скопированы'}catch(e){prompt('Скопируйте координаты:',value)}});
$('resetLayout').addEventListener('click',()=>{localStorage.removeItem(layoutKey());localStorage.removeItem(helperLayoutKey());localStorage.removeItem(helperStageLayoutKey());layoutItems.forEach(item=>{['left','top','right','bottom','width','height','z-index','visibility'].forEach(prop=>item.style.removeProperty(prop));['--layout-x','--layout-y','--layout-scale'].forEach(prop=>item.style.removeProperty(prop))});applyLayout();selectLayoutItem(layoutItems[0]);$('layoutStatus').textContent='возвращена стандартная расстановка'});

...
function readLayout(){try{return JSON.parse(localStorage.getItem(layoutKey())||'{}')}catch(e){return {}}}
...
$('resetLayout').addEventListener('click',()=>{localStorage.removeItem(layoutKey());layoutItems.forEach(item=>['left','top','right','bottom','width','height'].forEach(prop=>item.style.removeProperty(prop)));selectLayoutItem(null);$('layoutStatus').textContent='позиции и размеры сброшены'});

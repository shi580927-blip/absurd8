...
function renderAd(){const remaining=Math.max(0,state.adBonusUntil-Date.now());$('rewardedAd').classList.toggle('active',remaining>0);$('rewardedAd').disabled=remaining>0;$('rewardedAd').textContent=remaining>0?`🐟 Наедание ×3 · ${Math.ceil(remaining/60000)} мин.`:'🎬 Ускорить наедание ×3'}
...
$('rewardedAd').addEventListener('click',()=>{if(state.adBonusUntil>Date.now())return;if(!ysdk?.adv){$('phrase').textContent='Реклама будет доступна после запуска игры на Яндекс Играх.';return}$('rewardedAd').disabled=true;$('phrase').textContent='Шеф выбирает рекламный ролик, достойный его аппетита…';let rewarded=false;ysdk.adv.showRewardedVideo({callbacks:{onOpen:()=>{$('adStatus').textContent='Просмотр начался. Ускорение будет выдано после завершения.'},onRewarded:()=>{rewarded=true;state.adBonusUntil=Date.now()+5*60*1000;save()},onClose:()=>{$('phrase').textContent=rewarded?'Добавка одобрена. Шеф наедается втрое быстрее.':'Просмотр не завершён — добавку не выдали.';render(true)},onError:()=>{$('phrase').textContent='Сейчас реклама недоступна. Попробуйте немного позже.';$('rewardedAd').disabled=false}}})});

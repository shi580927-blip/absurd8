...
async function initYandexSDK(){try{if(window.YaGames){ysdk=await YaGames.init();ysdk.on?.('game_api_pause',handlePlatformPause);ysdk.on?.('game_api_resume',handlePlatformResume);announceGameReady()}}catch(error){ysdk=null}}
initYandexSDK();
const careRequests={
...
$('treats').addEventListener('click',e=>{const button=e.target.closest('.treat-card');if(!button)return;const index=+button.dataset.treat;if(isTreatUnlocked(index))return;e.stopImmediatePropagation();const treat=carpetFood[index];showRewardedAction({event:'treat_unlock',onReward:()=>{state.adTreatUnlocks.push(index);if(state.treatWish?.index===index){state.treatWish=null;state.nextTreatWish=Date.now()+(6+Math.random()*4)*60000}},success:`«${treat.name}» открыто навсегда. Теперь его можно купить за рыбов.`})},true);
...
function testDishReaction(refuse){showPanel(null,'openFeed');const cat=$('cat'),bowl=$('bowl'),message=refuse?'Шеф демонстративно отвернулся. Блюдо и рыбовы остались на месте.':'Шеф принял блюдо. Ковёр официально получил статус ресторана.';cat.classList.add(refuse?'refuses':'feasting');bowl.classList.add(refuse?'refused':'served');playSound(refuse?'cat-soft':'cat-happy',.7);previousThoughtMessage=message;$('phrase').textContent=message;showCatThought(refuse?'fussy':'happy',refuse?(Math.random()<.22?'Ой, бабочка… Шеф уже забыл, что заказывал.':'Шеф передумал. Унесите это немедленно.'):'Котик доволен. Можно продолжать обслуживание.');setTimeout(()=>{cat.classList.remove('refuses','feasting');bowl.classList.remove('refused','served')},900)}

...
let masterOrientationPaused=false;
addEventListener('message',event=>{if(event.source!==parent||event.origin!==location.origin||event.data?.type!=='master-orientation')return;masterOrientationPaused=!!event.data.paused;syncOrientation()});
function gameIsPaused(){return adPlaying||platformPaused||masterOrientationPaused||!brandIntroFinished||document.visibilityState==='hidden'||needsLandscape()}
...
const catReactionImages={happy:'assets/images/reactions/reaction-happy.webp',fussy:'assets/images/reactions/reaction-fussy.webp',stunned:'assets/images/reactions/reaction-stunned.webp',shock:'assets/images/reactions/reaction-shock.webp',pleading:'assets/images/reactions/reaction-pleading.webp',hunting:'assets/images/reactions/reaction-hunting.webp',judging:'assets/images/reactions/reaction-judging.webp',innocent:'assets/images/reactions/reaction-innocent.webp',trash:'assets/images/reactions/reaction-trash.webp',why:'assets/images/reactions/reaction-why.webp'};
const catReactionNames={happy:['Довольный Шеф','Satisfied Chef'],fussy:['Привередливый Шеф','Fussy Chef'],stunned:['Ошеломлённый Шеф','Stunned Chef'],shock:['Удивлённый Шеф','Surprised Chef'],pleading:['Голодный Шеф','Hungry Chef'],hunting:['Охотничий взгляд Шефа','Chef on the hunt'],judging:['Осуждающий Шеф','Judging Chef'],innocent:['Невиновный Шеф','Innocent Chef'],trash:['Шеф отвергает подачу','Chef rejects the serving'],why:['Шеф требует объяснений','Chef demands an explanation']};
...
function buffetImage(active){if(!active)return null;const clicks=state.buffetClicks[active.index]||0,[halfAt,emptyAt]=active.treat.stageClicks;const stage=clicks>=emptyAt?'empty':clicks>=halfAt?'half':'full';return `assets/images/buffets/${active.treat.buffetKey}-${stage}.webp`}
...
const layoutKey=()=>`absurd8-layout-v8-landscape`;
...
function readLayout(){const profile=layoutProfile();if(!layoutEditorMode)return {...defaultLayouts[profile]};try{return {...defaultLayouts[profile],...JSON.parse(localStorage.getItem(layoutKey())||'{}')}}catch(e){return {...defaultLayouts[profile]}}}

...
const layoutKey=()=>`absurd8-layout-${innerWidth>=760?'desktop':'mobile'}`;
const defaultLayouts={desktop:{grandmaHelper:{left:7.58,top:0,width:24.09},mouseDecor:{left:85.44,top:50.5,width:12.82},boxDecor:{left:68.05,top:68.24,width:30.04},chefHelper:{left:63.67,top:4.44,width:36.33},deliveryHelper:{left:0,top:45.64,width:30.22},laserDecor:{left:33.82,top:89.3,width:1.74}},mobile:{}};
let selectedLayoutItem=null;
function readLayout(){const device=innerWidth>=760?'desktop':'mobile';try{return {...defaultLayouts[device],...JSON.parse(localStorage.getItem(layoutKey())||'{}')}}catch(e){return {...defaultLayouts[device]}}}
function applyLayout(){const saved=readLayout();layoutItems.forEach(item=>{const pos=saved[item.id];if(!pos)return;item.style.left=`${pos.left}%`;item.style.top=`${pos.top}%`;item.style.right='auto';item.style.bottom='auto';item.style.width=`${pos.width}%`;item.style.height=item.id==='laserDecor'?`${pos.width}%`:'auto'})}
function saveLayoutItem(item){const stageRect=layoutStage.getBoundingClientRect(),rect=item.getBoundingClientRect(),saved=readLayout();saved[item.id]={left:+((rect.left-stageRect.left)/stageRect.width*100).toFixed(2),top:+((rect.top-stageRect.top)/stageRect.height*100).toFixed(2),width:+(rect.width/stageRect.width*100).toFixed(2)};localStorage.setItem(layoutKey(),JSON.stringify(saved))}
...
layoutItems.forEach(item=>item.addEventListener('pointerdown',e=>{if(!document.querySelector('.game').classList.contains('layout-mode'))return;e.preventDefault();selectLayoutItem(item);item.setPointerCapture(e.pointerId);const stageRect=layoutStage.getBoundingClientRect(),itemRect=item.getBoundingClientRect(),dx=e.clientX-itemRect.left,dy=e.clientY-itemRect.top;const move=ev=>{const rect=item.getBoundingClientRect(),left=Math.max(0,Math.min(stageRect.width-rect.width,ev.clientX-stageRect.left-dx)),top=Math.max(0,Math.min(stageRect.height-rect.height,ev.clientY-stageRect.top-dy));item.style.left=`${left/stageRect.width*100}%`;item.style.top=`${top/stageRect.height*100}%`;item.style.right='auto';item.style.bottom='auto';$('layoutStatus').textContent=`${item.dataset.layoutName}: ${Math.round(left/stageRect.width*100)}%, ${Math.round(top/stageRect.height*100)}%`};const done=()=>{item.removeEventListener('pointermove',move);saveLayoutItem(item)};item.addEventListener('pointermove',move);item.addEventListener('pointerup',done,{once:true});item.addEventListener('pointercancel',done,{once:true})}));
function resizeSelected(delta){if(!selectedLayoutItem)return;const stageWidth=layoutStage.getBoundingClientRect().width,current=selectedLayoutItem.getBoundingClientRect().width/stageWidth*100,next=Math.max(3,Math.min(55,current+delta));selectedLayoutItem.style.width=`${next}%`;selectedLayoutItem.style.height=selectedLayoutItem.id==='laserDecor'?`${next}%`:'auto';saveLayoutItem(selectedLayoutItem);$('layoutStatus').textContent=`${selectedLayoutItem.dataset.layoutName}: размер ${Math.round(next)}%`}
$('sizeDown').addEventListener('click',()=>resizeSelected(-2));$('sizeUp').addEventListener('click',()=>resizeSelected(2));
$('copyLayout').addEventListener('click',async()=>{const value=JSON.stringify({device:innerWidth>=760?'desktop':'mobile',positions:readLayout()},null,2);try{await navigator.clipboard.writeText(value);$('layoutStatus').textContent='координаты и размеры скопированы'}catch(e){prompt('Скопируйте координаты:',value)}});
$('resetLayout').addEventListener('click',()=>{localStorage.removeItem(layoutKey());layoutItems.forEach(item=>['left','top','right','bottom','width','height'].forEach(prop=>item.style.removeProperty(prop)));applyLayout();selectLayoutItem(null);$('layoutStatus').textContent='возвращена стандартная расстановка'});

...
const AD_DRAWER_COOLDOWN=120000;
let lastAdDrawerOpen=+(localStorage.getItem('absurd8-last-ad-drawer')||0);
function peekAdDrawer(){$('adDrawer').classList.add('peek')}
function showAdDrawer(){if(adPlaying)return false;const remaining=AD_DRAWER_COOLDOWN-(Date.now()-lastAdDrawerOpen);if(remaining>0){const seconds=Math.ceil(remaining/1000);$('phrase').textContent=`Реклама — раз в 2 минуты. Ещё ${seconds} сек.`;$('adStatus').textContent=`Повторно можно через ${seconds} секунд.`;playSound('error',.45);return false}lastAdDrawerOpen=Date.now();localStorage.setItem('absurd8-last-ad-drawer',String(lastAdDrawerOpen));$('adDrawer').classList.remove('peek');return true}

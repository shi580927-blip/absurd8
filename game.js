...
function testDishReaction(refuse){showPanel(null,'openFeed');const cat=$('cat'),bowl=$('bowl');cat.classList.add(refuse?'refuses':'feasting');bowl.classList.add(refuse?'refused':'served');playSound(refuse?'cat-soft':'cat-happy',.7);$('phrase').textContent=refuse?'Шеф демонстративно отвернулся. Блюдо и рыбовы остались на месте.':'Шеф принял блюдо. Ковёр официально получил статус ресторана.';setTimeout(()=>{cat.classList.remove('refuses','feasting');bowl.classList.remove('refused','served')},900)}

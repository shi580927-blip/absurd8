...
function applyStaticTranslations(){
  ...
  $('openCare').ariaLabel=L('Состояние Шефа','Chef’s status');$('openSettings').ariaLabel=L('Настройки','Settings');$('cat').ariaLabel=L('Покормить кота','Feed the cat');$('roomEvent').ariaLabel=L('Собрать случайный бонус','Collect a random bonus');$('rewardedAd').ariaLabel=L('Показать предложение или посмотреть рекламу','Show offer or watch ad');
...
function renderAd(){const cooldown=Math.max(0,AD_WATCH_COOLDOWN-(Date.now()-lastAdWatch)),boost=Math.max(0,state.adBonusUntil-Date.now()),button=$('rewardedAd');const clock=ms=>{const s=Math.ceil(ms/1000);return Math.floor(s/60)+':'+String(s%60).padStart(2,'0')};button.classList.toggle('active',boost>0);button.disabled=adRequestPending;button.innerHTML=adButton(boost>0?'×3 · '+clock(boost):'🎬 ×3',adRequestPending?L('Загрузка…','Loading…'):cooldown>0?L('Реклама через ','Next ad in ')+clock(cooldown):L('За рекламу','Watch ad'));button.ariaLabel=L('Ускорение за рекламу','Boost with an ad')}

...
function applyStaticTranslations(){
  ...
  $('openCare').ariaLabel=L('Состояние Шефа','Chef’s status');$('openSettings').ariaLabel=L('Настройки','Settings');$('cat').ariaLabel=L('Покормить кота','Feed the cat');$('roomEvent').ariaLabel=L('Собрать случайный бонус','Collect a random bonus');$('rewardedAd').ariaLabel=L('Показать предложение или посмотреть рекламу','Show offer or watch ad');$('adClose').ariaLabel=L('Скрыть предложение','Hide offer');
...
function adButton(title,subtitle){return `<span class="action-icon" aria-hidden="true">🎬</span><span class="action-copy"><b>${title}</b><small>${subtitle}</small></span>`}
...
$('rewardedAd').addEventListener('click',e=>{e.stopPropagation();if($('adDrawer').classList.contains('peek'))showAdDrawer();else mainRewardedAdClick()});
